...
class Uni:
    def uni_method(self, g1, g2, g3, g4):
        print("Я забыл, ты где учишься. В мирэа или мгу?(мирэа/мгу)")
        g1 = str(input())
        if g1.lower().replace(" ", "") == "мгу":
            return "До встречи"
            ...
            if g2.lower().replace(" ", "") == "нет":
                ...
                print("Java и Js это одно и тоже?(да/нет)")
                g3 = str(input())
                if g3.lower().replace(" ", "") == "нет":
                    return "Отлично, ты знаешь что-то"
                elif g3.lower().replace(" ", "") == "да":
                    print("Может тогда перестать учиться(да/да)")
                    g4 = str(input())
